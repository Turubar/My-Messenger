...
namespace Application.Services
{
    public class UsersService
    {
        ...
        public async Task<Result<User>> Registration(string login, string password)
        {
            ...
            if (existingUser.IsSuccess) return Result.Failure<User>("Пользователь с таким логином уже существует");

            // Добавляем пользователя в базу данных
            var user = await _userRepository.Add(newUser.Value);
            if (user.IsFailure) return Result.Failure<User>("Не удалось зарегистрировать пользователя");
